...
class pos_sale_report_template(models.AbstractModel):
    _name = 'report.pos_retail_standard.pos_sale_report_template'
    _description = "Template Report of sale"

    @api.model
    def _get_report_values(self, doc_ids, data=None):
        report = self.env['ir.actions.report']._get_report_from_name('pos_retail_standard.pos_sale_report_template')
        return {
...
class pos_sale_report(models.TransientModel):
    ...
    def print_receipt(self):
        ...
        return self.env.ref('pos_retail_standard.report_pos_sales_pdf').report_action(self, data=datas)
