odoo.define('pos_retail_standard.ButtonSetPickingType', function (require) {
    'use strict';
        ...
        async onClick() {
            ...
            if (list.length > 0) {
                let {confirmed, payload: pickingType} = await this.showPopup('SelectionPopup', {
                    title: this.env._t('Current redeem stock from: ') + OrderLocationSelected.display_name + this.env._t(' . Are you want change Source Location of Delivery Order of current Order ?'),
                    list: list,
                })
                if (confirmed) {
