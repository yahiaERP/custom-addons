odoo.define('pos_retail_standard.ButtonCreateSaleOrder', function (require) {
    'use strict';
    ...
    class ButtonCreateSaleOrder extends PosComponent {
        ...
        async onClick() {
            ...
            const {confirmed, payload: values} = await this.showPopup('PopUpCreateSaleOrder', {
                title: this.env._t('Create Sale(Quotation)/Reservation Order [Sale Order]'),
                order: order,
            ...
            if (confirmed) {
                ...
                let value = {
                    ...
                    book_order: true,
                    ean13: order.ean13,
                    delivery_name: order.get_client().name,
                ...
                let {confirmed, payload: confirm} = await this.showPopup('ConfirmPopup', {
                    title: this.env._t('Quotation [Sale Order] : ') + result['name'] + this.env._t(' created successfully'),
