odoo.define('pos_retail_standard.PosOrderDetail', function (require) {
    'use strict';
    ...
    class PosOrderDetail extends PosComponent {
        constructor() {
            ...
            useListener('update-customer-name', () => this.updateCustomerName());
        }

        async downloadOrderReport() {
            this.env.pos.do_action('pos_retail_standard.report_pos_order', {
                additional_context: {
        ...
        async updateCustomerName() {
            let {confirmed, payload: newName} = await this.showPopup('TextAreaPopup', {
                title: this.env._t('What new Name replace old name :  ') + this.props.order.partner.name,
                startingValue: this.props.order.partner.name
            })
            if (confirmed) {
                let {confirmed, payload: result} = await this.showPopup('ConfirmPopup', {
                    title: this.env._t('Warning, Are you sure change Name of Customer'),
                    body: this.env._t('From name [ ') + this.props.order.partner.name + this.env._t(' ] to name [ ') + newName + ' ]',
                })
                if (confirmed) {
                    let partnerId = await this.rpc({
                        ...
                        args: [{
                            'id': this.props.order.partner.id,
                            'name': newName
                        }],
                    })
                    if (partnerId) {
                        let client = this.env.pos.db.get_partner_by_id(this.props.order.partner.id)
                        client['name'] = newName
                        this.render()
                    }
                }
