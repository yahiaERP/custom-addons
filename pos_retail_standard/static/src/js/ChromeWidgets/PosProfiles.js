odoo.define('pos_retail_standard.PosProfiles', function (require) {
    'use strict';
    ...
    class PosProfiles extends PosComponent {
        async onClick() {
            ...
            const posProfiles = await this.env.pos.getDatasByModel('pos.config', [['allow_change_pos_profile', '=', true], ['id', '!=', this.env.pos.config.id]], ['name', 'current_session_state'])
            if (!posProfiles) {
                return false
            }
