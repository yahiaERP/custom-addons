    ...
    const RetailSearchBar = (SearchBar) =>
        class extends SearchBar {
            ...
            onKeyup(event) {
                ...
                if (this.props.displayClearSearch && !['ArrowUp', 'ArrowDown', ''].includes(event.code)) { // only for products screen. When keyup event called here, trigger search input and filter products data from search box
                    this.trigger('clear-search-product-filter')
                    if (event.code != '') {
                        this.trigger('update-search', event.target.value);
