...
class res_partner(models.Model):
    ...
    def write(self, vals):
        if vals.get('birthday_date', None):
            birthday_date = datetime.strptime(vals.get('birthday_date'), "%d-%m-%Y")
            vals.update({'birthday_date': birthday_date})
        ...
        partnert_ids = []
        for p in self:
            partnert_ids.append(p.id)
        sessions = self.env['pos.session'].search([
            ('state', '=', 'opened')
        ])
        for s in sessions:
            self.env['bus.bus']._sendone(s.user_id.partner_id, 'bus.sync.partner', {
                'partner_ids': partnert_ids
            })
        return res
